...
class PoliciesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create policy" do
    assert_difference('Policy.count') do
      post policies_url, params: { policy: { content: @policy.content, title: @policy.title } }
    end
  ...
  test "should update policy" do
    patch policy_url(@policy), params: { policy: { content: @policy.content, title: @policy.title } }
